...
def class_balanced_cross_entropy_loss1(output, label, size_average=True, batch_average=True):
    """Define the class balanced cross entropy loss to train the network
    Args:
    output: Output of the network
    label: Ground truth label
    ...
    loss_pos = torch.sum(-torch.mul(labels, torch.log(prob)))
    loss_neg = torch.sum(-torch.mul(1.0 - labels, torch.log(1.0 - prob)))
...
def plane_embedding_loss(embedding, plane_gids, num_unique_plane, device, t_pull=0.5, t_push=1.5):
    c, plane_num = embedding.size()
    embeddings = []
    for i in range(num_unique_plane):
        m = plane_gids == i
        feature = torch.transpose(torch.masked_select(embedding, m).view(c,-1),0,1)
    ...
    pull_loss /= int(num_unique_plane)

    if num_unique_plane == 1:
        return pull_loss, torch.zeros(1).to(device)

    # inter-plane loss
    centers = torch.cat(centers, dim=0)
    A = centers.repeat(1, int(num_unique_plane)).view(-1, c)
    B = centers.repeat(int(num_unique_plane), 1)
    distance = torch.norm(A - B, 2, dim=1).view(int(num_unique_plane), int(num_unique_plane))

    # select pair wise distance from distance matrix
    eye = torch.eye(int(num_unique_plane)).to(device)
...
def hinge_embedding_loss(embedding, segmentation, device, t_pull=0.5, t_push=1.5):
    b, c, h, w = embedding.size()
    assert(b == 1)

    num_planes = segmentation.size(1)
...
def Q_loss(param, gt_point, mask):
    #print(param.size(), gt_point.size(),mask.size())
    mask = mask.view(1, -1)
    plane_num = param.size(1)

    loss = 0.
    for i in range(plane_num):
        m = (mask == i+1)
        valid_point = torch.masked_select(gt_point, m).view(3, -1)

        loss += torch.mean(torch.abs(torch.sum(param[0:3,i:i+1]*valid_point, dim=0, keepdim=True) - param[3,i]))
...
def Q_loss_old(param, k_inv_dot_xy1, gt_depth):
    '''
    ...
    k_inv_dot_xy1 = k_inv_dot_xy1.view(3, h*w)
    ...
    # select valid depth
    mask = gt_depth > 1e-4#gt_depth != 0.0
...
#     labels = torch.randint(2,(1, 4,5))

#     print(class_balanced_cross_entropy_loss(output.float(), labels.float()))
#     print(class_balanced_cross_entropy_loss1(output.float(), labels.float()))
