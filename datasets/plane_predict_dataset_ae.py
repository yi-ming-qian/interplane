...
class PlaneDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        while 1: 
            ...
            matched_single = []
            # load ae results
            ae_results = np.load(self.dataFolder + f"/planeae_result/{sceneIndex}_{imageIndex}.npz")
            pd_masks = ae_results["ae_masks"]>0.5
            pd_planes = ae_results["ae_planes"]
            for i, pd_m in enumerate(pd_masks):
                max_iou = 0.
                max_id = -1
                for j, gt_m in enumerate(gt_masks):
                    iou = eval_iou(gt_m, pd_m)
                    if iou>0.3 and iou>max_iou:
                        max_iou = iou
                        max_id = j
                if max_id<0: # not matched
                    tg_planes.append(np.zeros(4))
                    tg_masks.append(np.full((480,640), False))
                    matched_single.append(False)
                else:
                    tg_planes.append(gt_planes[max_id])
                    tg_masks.append(gt_masks[max_id])
            ...
            for i in range(num_planes):
                ...
                for j in range(i+1, num_planes):
                    ...
                    # check contact or not
                    line_mask, contact_flag = self.clip_line_segment(intersect_mask, pm, qm, 15)
        ...
        return sample
    def match_ae_results(self, ae_masks, ae_planes, tg_masks, tg_planes, matched_single):
        ae_planes_sorted = np.zeros_like(tg_planes)
        for i, tg_m in enumerate(tg_masks):
            if matched_single[i]==False:
                continue
            max_iou = 0.
            max_id = 0
            for j, ae_m in enumerate(ae_masks):
                iou = eval_iou(tg_m, ae_m>0.5)
                if iou>max_iou:
                    max_iou=iou
                    max_id = j
            ae_planes_sorted[i] = ae_planes[max_id]
        return ae_planes_sorted
