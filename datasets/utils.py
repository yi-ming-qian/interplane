...
# def write_obj_file(filename, points, faces):
...
def write_obj_file(filename, points, normals, textures, faces):
    mtlfile = filename.replace('.obj', '.mtl')
    _, mtlname = os.path.split(mtlfile)
    with open(filename, 'w') as f:
        ...
        for pt in points:
            f.write(f'v {pt[0]} {pt[1]} {pt[2]}\n')
        for nr in normals:
            f.write(f'v {nr[0]} {nr[1]} {nr[2]}\n')
        for te in textures:
            f.write(f'vt {te[0]} {te[1]}\n')
        f.write('usemtl mymaterial\n')
        for fa in faces:
            f.write(f'f {fa[0]}/{fa[0]}/{fa[0]} {fa[1]}/{fa[1]}/{fa[0]} {fa[2]}/{fa[2]}/{fa[0]}\n')
    with open(mtlfile, 'w') as f:
        f.write('newmtl mymaterial\n')
        #f.write('Ka spectral ident.rfl 1\n')
        #f.write('Kd spectral ident.rfl 1\n')
        #f.write('illum 1\n')
        f.write('map_Ka image.jpg\n')
        #f.write('map_Kd image_s.png')



def writePointCloud2(filename, image, points):
    cv2.imwrite(filename.replace('.ply','.png'), image)
    if points.shape[0] == 3:
    ...
    writePointCloud(filename, np.concatenate([points,image],axis=1))
...
"""
        ...
        pass
    return
def writePointCloud_nocolor(filename, point_cloud):
    with open(filename, 'w') as f:
        header = """ply
format ascii 1.0
element vertex """
...
def transform_plane_12g(transformation, planes, scale=False):
    ...
    if scale:
        tmp = np.linalg.norm(newNormals, axis=-1, keepdims=True)
        newNormals /= tmp
        newOffsets /= tmp

    return newNormals * newOffsets

def transform_plane_g2l(transformation, planes, scale=False):
    #planeOffsets = np.linalg.norm(planes, axis=-1, keepdims=True)
    #planeNormals = planes / np.maximum(planeOffsets, 1e-4)
    #planeNormals /= np.linalg.norm(planeNormals, axis=-1, keepdims=True)
    planeNormals = planes[:,0:3]
    planeOffsets = planes[:,3:4]

    rotation_mat = transformation[0:3,0:3]
    rotation_transpose = np.transpose(rotation_mat) # actually inverse
    if scale:
        rotation_transpose = np.diag(1.0/np.diag(rotation_mat))
    translation_vec = transformation[0:3, 3:4]
...
def calc_points_by_param_map(param, camera, max_depth=10, transformation=[]):
    if param.shape[0] == 4:
    ...
    WorldXYZ = np.matmul(rotation_transpose, CameraXYZ-translation_vec) # 3 x (height*width)
    return WorldXYZ

def calc_points_by_depth(depth, camera, transformation=[]):
    ...
    ranges = ranges.reshape(-1, 3)
    CameraXYZ = depth * ranges
    if len(transformation) is 0:
        return CameraXYZ
    CameraXYZ = CameraXYZ.transpose()
